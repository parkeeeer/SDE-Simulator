...
TEST_CASE("basic expression parsing test"){
    ...
    Parser<double> p(env,(std::move(tokens)));
    AST<double> ast = p.parse();
    CHECK(ast.eval(0,0) == doctest::Approx(4.0 + 2.0 * 7.0 - 1.0 / std::pow(9.0, 5.0)));
}
...
TEST_CASE("test exp associativity(right)"){
    ...
    Parser<double> p(env,std::move(tokens));
    AST<double> ast = p.parse();
    CHECK(ast.eval(0,0) == doctest::Approx(std::pow(2.0, std::pow(3.0, 2.0))));
}
...
TEST_CASE("unary start test"){
    ...
    Parser<double> p(env,(std::move(tokens)));
    AST<double> ast = p.parse();
    CHECK(ast.eval(0,0) == doctest::Approx(-3.0 + -(-2.0)));
}
...
TEST_CASE("parameter test"){
    ...
    Parser<double> p(env,std::move(tokens));
    AST<double> ast = p.parse();
    CHECK(ast.eval(0,0) == doctest::Approx(4.0 + 2.0 * 7.0));
}

TEST_CASE("test state variables"){
    Lexer l("X + 2 * t * dt - X *8 * t");
    auto tokens = l.lex();
    Environment<double> env;
    env.set_dt(.05);
    Parser<double> p(env,(std::move(tokens)));
    AST<double> ast = p.parse();
    CHECK(ast.eval(3.0,4.0) == doctest::Approx(3.0 + 2.0 * 4.0 * 0.05 - 3.0 * 8.0 * 4.0));
}
...
TEST_CASE("test implicit multiplication"){
    ...
    Parser<double> p(env,std::move(tokens));
    AST<double> ast = p.parse();
    CHECK(ast.eval(1.0,0) == doctest::Approx(2.0 * 1.0 + 3.0 * (4.0 + 1.0) + std::sin(1.0) * std::cos(1.0)));
}
...
TEST_CASE("test nested function calls"){
    ...
    Parser<double> p(env,std::move(tokens));
    AST<double> ast = p.parse();
    CHECK(ast.eval(0,0) == doctest::Approx(std::sin(std::cos(0.0)) + std::log(std::exp(1.0)) + std::sqrt(4.0 + std::sqrt(16.0))));
}
